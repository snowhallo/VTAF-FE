      ...
      <div className="relative">
        ...
        <input
          ...
          className={`
            ...
            rounded border text-sm text-slate-800 placeholder:text-slate-400 bg-white
...
function SocialButtons({ action }: { action: "Đăng nhập" | "Đăng ký" }) {
  return (
    <div className="space-y-2.5">
      <button type="button" className="w-full h-11 flex items-center gap-3 px-4 border border-slate-300 rounded text-sm text-slate-700 bg-white hover:bg-slate-50 transition-colors">
        <FacebookIcon /><span>{action} với Facebook</span>
      </button>
      <button type="button" className="w-full h-11 flex items-center gap-3 px-4 border border-slate-300 rounded text-sm text-slate-700 bg-white hover:bg-slate-50 transition-colors">
        <GoogleIcon /><span>{action} với Google</span>
      </button>
    </div>
  )
}
...
function Divider({ text }: { text: string }) {
  return (
    <div className="relative">
      ...
      <div className="relative flex justify-center"><span className="bg-white px-3 text-xs text-slate-400">{text}</span></div>
    </div>
  )
}
...
function SubmitBtn({ label, loading, loadingLabel }: { label: string; loading: boolean; loadingLabel: string }) {
  return (
    <button type="submit" disabled={loading}
      className="w-full h-12 bg-slate-900 hover:bg-slate-700 text-white font-bold rounded flex items-center justify-center gap-2 transition-all disabled:opacity-60">
      {loading
...
function TermsText() {
  return (
    <p className="text-center text-[11px] text-slate-400 leading-relaxed">
      ...
      <Link href="/terms" target="_blank" className="underline hover:text-slate-600">Điều khoản &amp; Điều kiện</Link>
      {" "}và{" "}
      <Link href="/privacy" target="_blank" className="underline hover:text-slate-600">Chính sách bảo mật</Link>
...
function ForgotPasswordFlow({ onBack }: { onBack: () => void }) {
  ...
  const methodLabel = method === "phone" ? "số điện thoại" : "email"
    ...
    <div className="space-y-5">
      ...
      {step === "choose" && (
        <>
              ...
              {(["phone", "email"] as ResetMethod[]).map((m) => (
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-3 rounded-lg border-2 text-sm font-medium transition-all ${
                    method === m ? "border-[#006B3F] bg-[#006B3F]/5 text-[#006B3F]" : "border-slate-200 text-slate-600 hover:border-slate-300"
                  }`}
                >
                  {m === "phone"
          ...
          <button onClick={onBack} className="w-full text-center text-sm text-slate-500 hover:text-slate-700 flex items-center justify-center gap-1">
        ...
        <>
          ...
          <form onSubmit={handleVerifyOtp} className="space-y-4">
            ...
            <div className="flex gap-2 justify-center" onPaste={handleOtpPaste}>
              {otp.map((digit, idx) => (
                <input
                  ...
                  className={`w-11 h-13 py-3 text-center text-xl font-bold rounded-lg border-2 transition-all focus:outline-none
            ...
            <div className="text-center">
              {secondsLeft > 0 ? (
                <span className={`text-sm font-semibold ${secondsLeft <= 60 ? "text-red-500" : "text-slate-600"}`}>
                  Mã hết hạn sau{" "}
                  <span className="font-mono">{formatTime(secondsLeft)}</span>
                </span>
              ) : (
                <p className="text-red-500 text-sm font-medium">Mã đã hết hạn!</p>
              )}
            </div>
            ...
            <div className="text-center">
              {canResend ? (
                <button type="button" onClick={handleResend} disabled={loading}
                  className="text-sm text-[#006B3F] hover:underline font-medium disabled:opacity-50">
                  Gửi lại mã
                </button>
              ) : (
          ...
          <button onClick={() => setStep("choose")} className="w-full text-center text-sm text-slate-500 hover:text-slate-700 flex items-center justify-center gap-1">
      ...
      {step === "success" && (
        <div className="flex flex-col items-center gap-4 py-6 text-center">
          ...
          <div>
            <h3 className="font-bold text-slate-800 text-lg">Đổi mật khẩu thành công! 🎉</h3>
            <p className="text-sm text-slate-500 mt-1">Đang chuyển hướng về trang chủ...</p>
          </div>
          <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
            <div className="h-full bg-[#006B3F] rounded-full animate-[progress_2.5s_linear_forwards]" />
          </div>
          <Link href="/" className="text-sm text-[#006B3F] hover:underline">
            Nhấn đây nếu không tự chuyển hướng →
          </Link>
        </div>
      )}
    </div>
...
function LoginForm({ onForgot }: { onForgot: () => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Divider text="Hoặc đăng nhập bằng tài khoản" />

      <form onSubmit={handleSubmit} className="space-y-3">
        <Field placeholder="Tên đăng nhập" value={username} onChange={setUsername} error={errors.username} icon={User} autoComplete="username" />
        <Field
        ...
        <SubmitBtn label="Đăng nhập" loading={loading} loadingLabel="Đang đăng nhập..." />
        <TermsText />
      </form>
    </div>
  )
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div>
          ...
          {!isUsernameValid && (
            <p className="text-[11px] text-amber-600 mt-1 leading-relaxed">
              💡 Hướng dẫn: Tên đăng nhập phải có ít nhất 4 ký tự, chỉ gồm các chữ cái (không dấu), chữ số và dấu gạch dưới (_). Không chứa khoảng trắng hoặc ký tự đặc biệt khác.
            </p>
          )}
        </div>
        ...
        <div>
          ...
          {!isEmailValid && (
            <p className="text-[11px] text-amber-600 mt-1 leading-relaxed">
              💡 Hướng dẫn: Email phải đúng định dạng chuẩn (ví dụ: quynhantai@gmail.com).
            </p>
          )}
        </div>
        ...
        <div>
          ...
          {!isPhoneValid && (
            <p className="text-[11px] text-amber-600 mt-1 leading-relaxed">
              💡 Hướng dẫn: Số điện thoại phải có đúng 10 số và bắt đầu bằng các đầu số di động Việt Nam (03, 05, 07, 08, 09).
            </p>
          )}
        </div>
        ...
        <div>
          ...
          {/* Real-time Checklist for Password Safety */}
          {password.length > 0 && (
        ...
        <SubmitBtn label="Đăng ký" loading={loading} loadingLabel="Đang tạo tài khoản..." />
        <TermsText />
      </form>
    </div>
  )
...
export default function AuthPage() {
  ...
  const headerTitle: Record<Tab, string | null> = {
    login: null,
    register: null,
  ...
  return (
    <div className="min-h-screen bg-slate-200/80 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm bg-white rounded-xl shadow-xl overflow-hidden">
        ...
        {/* Logo — ẩn khi đang ở forgot flow (có header riêng) */}
        {tab !== "forgot" && (
          <div className="flex justify-center pt-6 pb-2">
            <Link href="/" className="flex flex-col items-center gap-2">
              <div className="relative w-12 h-12 overflow-hidden rounded-full border border-slate-200">
                <Image src="/Logo_quy.jpg" alt="Logo Quỹ" fill sizes="48px" className="object-cover" />
              </div>
              <div className="text-center leading-tight">
                <div className="font-[family-name:var(--font-playfair)] font-bold text-[12px] text-[#C21A30]">
        ...
        {tab !== "forgot" ? (
            ...
            {(["login", "register"] as Tab[]).map((t) => (
              <button key={t} onClick={() => setTab(t)}
                className={`flex-1 py-3 text-sm font-semibold transition-colors border-b-2 ${
                  tab === t ? "text-slate-900 border-slate-900" : "text-slate-400 border-transparent hover:text-slate-600"
                }`}
              >
          ...
          <div className="flex items-center gap-2 px-6 py-4 border-b border-slate-100">
            <Link href="/" className="relative w-8 h-8 overflow-hidden rounded-full border border-slate-200 shrink-0">
              <Image src="/Logo_quy.jpg" alt="Logo" fill sizes="32px" className="object-cover" />
            </Link>
